 ...
 * на выравнивание, при размере int = 4 в среднем - 1.5 байта.) При значении среднего размера слова 5-9 байт
...
// Вместительность дерева вместе со всеми заголовками.
const unsigned int usefulCapacityWBST = (capacityWordBST + 1) * sizeof(int);
...
// Средние потери на слово = счетчик + доп.атрибуты + средние потери на выравнивание + терминирующий строку ноль.
const double averageLoss = sizeof(int) + addFieldsSize + 0.5 * (sizeof(int) - 1) + 1.0;
...
union WordNode
{
    struct
    {
        ...
        char word[2];              // Строка. Выход за границы этого массива - запланированное и управляемое событие.
